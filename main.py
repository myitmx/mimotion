...
def login(user, password):
    if re.match(r'\w[-\w.+]*@([A-Za-z0-9][-A-Za-z0-9]+\.)+[A-Za-z]{2,14}', user):
...
def main(_user,_passwd, _step):
    ...
    url = 'https://api-mifit-cn.huami.com/v1/data/band_data.json?&t={}'.format(t)
    head = {
        "apptoken": app_token,
        "Content-Type": "application/x-www-form-urlencoded"
    }

    data = 'userid={}&last_sync_data_time=1597306380&device_type=0&last_deviceid=DA932FFFFE8816E7&data_json={}'.format(userid, data_json)

    response = requests.post(url, data=data, headers=head).json()
    # print(response)
    result = "[{}] 账号：{} 修改步数（{}）[ {} ]" . format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), user, step, response['message'])
    #print(result)
    return result
...
def get_app_token(login_token):
    url = "https://account-cn.huami.com/v1/client/app_tokens?app_name=com.xiaomi.hm.health&dn=api-user.huami.com%2Capi-mifit.huami.com%2Capp-analytics.huami.com&login_token={}".format(login_token)
    response = requests.get(url, headers=headers).json()
